...
<%
	...
	PetDescVO dVO = (PetDescVO) session.getAttribute("dVO");
...
<form action = "EditBoard.jsp" method="post" enctype="multipart/form-data"> 
	<div id = "main" class= "container mt-5 mb-5">
		<div id = "top" class ="row" style="padding:10px 120px 0px 120px">
				...
				<% } %>	
				  <input required name="title" id = "title" type="text" class="form-control" aria-label="Text input with dropdown button" value="<%=bVO.getBoard_title()%>">
				</div>
		</div>
		<div id = "center" class="row" style="padding:0px 120px 0px 120px">
			<div class="form-floating">
			  <textarea name = "content" class="form-control" id="content" value="" required><%=bVO.getBoard_content()%></textarea>
			</div>
		...
		<% if (!cType.equals("notice")) {%>
			<table class="table table-striped" height="80px" align="center">
			  <tr>
			  	...
			  	<td><input name="name" type="text" class="form-control"  style="height: 33px; width:200px;" value="<%=dVO.getPet_name()%>" required> *반려 이름을 적어주세요</td>
			  </tr>
			  
			  <tr>
			  	<td>종류</td>
			  	<td><input name="type" type="text" class="form-control"  style="height: 33px; width:200px;" value = "<%=dVO.getPet_type()%>"required> ex) 말티즈/러시안블루/카멜레온 etc..</td>
			  </tr>
			   <tr>
			  	<td>날짜</td>
			  	<td><input name="date" type="date" class="form-control"  style="height: 33px; width:200px;" value = "<%=dVO.getPet_date()%>" required> *분실 날짜</td>
			  </tr>
			   ...
			   <tr>
			  	<td>번호</td>
			  	<td><input name="tel" type="text" class="form-control"  style="height: 33px; width:200px;" value="<%=dVO.getPet_tel()%>" required> *항상 받으실 수 있는 번호를 적어주세요</td>
			  </tr>
			  <tr>
			  	<td>위치</td>
			  	<td><input name="location" type="text" class="form-control"  style="height: 33px; width:500px;" value="<%=dVO.getPet_location() %>" required>*잃어버리신 위치를 자세히 적어주세요</td>
			  </tr>
			</table>
	<% } %>
		</div>
		...
		<div class="row">
			...
			<div class="col-lg-5" style="padding:10px 130px 0px 0px" align="right">
				<input name="board_type" type="text" style="display:none;" value="find"> 
